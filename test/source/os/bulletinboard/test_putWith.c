...
static void test_PopulatesEntry(void) {
    ...
    os_PutWith(TEST_KEY, test_ctx.data, test_TimeoutAction, TEST_TICKS);
...
static void test_AcquiresContext(void) {
    setUp();
...
static void test_ZeroTicks_GoesToFifo(void) {
    ...
    os_PutWith(TEST_KEY, test_ctx.data, test_TimeoutAction, 0);
...
static void test_NoKey_SkipsCancel(void) {
    ...
    os_PutWith(OS_NO_KEY, test_ctx.data, test_TimeoutAction, TEST_TICKS);
...
static void test_AllocFails(void) {
    ...
    Assert_Called1(os_Fail, OS_FAIL_PUT_WITH_ALLOCATION);
    Assert_NotCalled(os_TimerAdd);
    Assert_NotCalled(os_FifoAdd);
    Assert_NotCalled(os_CancelPending);
    Assert_NotCalled(os_ContextAcquire);
}
